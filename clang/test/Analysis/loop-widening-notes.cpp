...
int test_for_bug_25609() {
  ...
  for (int i = 0;  // expected-note {{Loop condition is true.  Entering loop body}}                    
       ...
       ++i,        // expected-note {{Value assigned to 'p_a' (previous assignment invalidated as part of ExprEngine : Widen Loop)}}
       i < flag_a;
       ++i) {}
...
int while_analyzer_output() {
  ...
  while (flag_b-- > 0) { // expected-note {{Loop condition is true.  Entering loop body}} 
                         // expected-note@-1 {{Value assigned to 'num' (previous assignment invalidated as part of ExprEngine : Widen Loop)}}
...
int do_while_analyzer_output() {
  ...
  do {   // expected-note {{Loop condition is true. Execution continues on line 47}} 
    ...
    num--;
  } while (flag_c-- > 0); //expected-note {{Value assigned to 'num' (previous assignment invalidated as part of ExprEngine : Widen Loop)}}
  int local = 0;
...
int test_for_loop() {
  ...
  for (int i = 0;    // expected-note {{Loop condition is true.  Entering loop body}} 
       ...
       new int(10),  // expected-note {{Value assigned to 'num' (previous assignment invalidated as part of ExprEngine : Widen Loop)}}
       i < flag_d;
       ++i) {         
    ++num;
  }
...
int test_for_range_loop() {
  int arr[10] = {0};
  for(auto x : arr) { // expected-note {{Assigning value (previous assignment invalidated as part of ExprEngine : Widen Loop)}}
    ++x;
  }
